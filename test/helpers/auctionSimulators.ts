import  {getContract, getContractAddressByName } from '../contractUtils';

import BigNumber from '../bignumber';
import { DAI_NUMBER_OF_DIGITS, MKR_NUMBER_OF_DIGITS, RAD_NUMBER_OF_DIGITS } from '../constants/UNITS';
...
const ensureQueuedDebtEqual = async (expected: BigNumber) => {
    const contract = await getContract('MCD_VOW');
...
const overwriteProtocolOwnDaiBalance = async (amount: BigNumber) => {
    const daiOwnerAddress = await getContractAddressByName('MCD_VOW');
...
const ensureDaiIsZero = async () => {
    const vatContract = await getContract('MCD_VAT');
    const daiOwnerAddress = await getContractAddressByName('MCD_VOW');
...
const startDebtAuction = async () => {
    const vow = await getContract('MCD_VOW');
    await vow.flop();
};
...
const ensureDebtQueueEntry = async (expectedDai: BigNumber) => {
    const contract = await getContract('MCD_VAT');
    const owner = await getContract('MCD_VOW');
