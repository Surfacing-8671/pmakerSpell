...
contract CurveLPOracle {
    ...
    function poke() external payable {
        ...
        //pls price
        uint256 pricePls = OracleLike(orbs[0]).read();
        //wstEth price
        uint256 priceWsteth = OracleLike(orbs[1]).read();
        //balance of pls
        uint256 plsBalance = CurvePoolLike(pool).balances(0);
        //balance of wstEth
        uint256 wstEthBalance = CurvePoolLike(pool).balances(1);

        uint256 scaledPricePls = pricePls; // Now in 1e18 precision
        uint256 scaledPriceWsteth = priceWsteth;

        uint256 val0 = (scaledPricePls * plsBalance);

        uint256 val1 = (scaledPriceWsteth * wstEthBalance);

        uint256 totalSupply = TokenLike(CurvePoolLike(pool).lp_token())
            .totalSupply();

        // Compute the total pool value
        uint256 totalValue = val0 + val1; // Still in 1e36 precision

        // Scale result to 1e27 precision
        uint256 lpTokenValue = (totalValue / totalSupply); // (1e36 / 1e18) / 1e9 = 1e27

        Feed memory cur_ = nxt;
        cur = cur_;
        nxt = Feed(uint128(lpTokenValue), 1);
