...
contract UniOracle {
    ...
    constructor(address factory, address _pairLp) public {
        IUniswapV2Pair _pair = IUniswapV2Pair(_pairLp);
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
...
contract PulsexV1OracleFactory {
    ...
    function deploy(address _pairLp) external {
        address _pair = _pairLp;
        ...
        address _oracle = address(new UniOracle(FACTORY, _pair));
        lookups[_pair] = _oracle;
        _pairs.push(_pair);
        _oracles.push(_oracle);
    }

    function updateable(address _pair) external view returns(bool){
        return UniOracle(lookups[_pair]).updateable();
    }

    function update(address _pair) external {
        UniOracle(lookups[_pair]).update();
    }

    function quote(address _pair, address tokenIn, uint amountIn) external view returns (uint amountOut) {
        return UniOracle(lookups[_pair]).consult(tokenIn, amountIn);
    }
